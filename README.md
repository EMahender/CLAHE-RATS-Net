...
3. Patient-level split: Patients are split into training, validation, and testing sets to prevent leakage.
4. ROI extraction: Positive nodule slices are converted into 256 x 256 ROI crops with click/center heatmaps.
5. Stage 1: A click-guided attention U-Net produces a coarse nodule mask.
6. Stage 2: A residual refinement network uses the CLAHE ROI, click map, and Stage-1 probability map to refine the segmentation.
7. Evaluation: Dice, IoU, precision, recall, specificity, threshold analysis, size-wise analysis, and statistical tests are computed.
